<template>
    <div>
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>DriverId</v-list-item-title>
                    </v-list-item-content>
                </template>
        ...
        <DriverId v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import DriverId from '../DriverId.vue';

    export default {
        name: 'DriverIdManager',
        components: {
            DriverId
        },
        props: {
